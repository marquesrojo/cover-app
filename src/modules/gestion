...
function EditEventoModal({ticket,onClose,onSaved}){
  ...
  const [asignado,setAsignado]=useState(ticket.assigned_to||'')
  ...
  const save=async()=>{
    setSaving(true)
    const updates={status,severity,title,description:desc,resolution_notes:resolution,assigned_to:asignado||null,updated_at:new Date().toISOString()}
    ...
    <div style={{position:'fixed',inset:0,background:'#000b',zIndex:500,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{width:'100%',maxWidth:480,background:C.surface,border:`1px solid ${C.border}`,borderRadius:'12px 12px 0 0',padding:20,maxHeight:'90vh',overflowY:'auto',animation:'slideUp 0.25s ease'}}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
          <span className="mono" style={{fontSize:12,fontWeight:700,color:C.amber}}>EDITAR EVENTO</span>
        ...
        <Select label="Asignado a" value={asignado} onChange={setAsignado}
          options={[{value:'',label:'Sin asignar'},...usuarios.map(u=>({value:u.id,label:u.full_name}))]}/>
...
function EventosTab(){
  ...
  async function createEvento(){
    ...
    setSaving(true)
    const payload={plant_id:plantId,created_by:user.id,severity,title,description:desc,sector,status:'abierto',assigned_to:asignado||null}
    ...
    if(!error){
      if(photo?.file){
        try{
          ...
          const {data:{publicUrl}}=supabase.storage.from('ticket-photos').getPublicUrl(up.path)
      ...
      setShowNew(false);setPlantId('');setSeverity('');setTitle('');setDesc('');setSector('');setAsignado('');setPhoto(null)
    }
    setSaving(false)
  }
  ...
  return(
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
        <div className="mono" style={{fontSize:10,color:C.muted,letterSpacing:'0.1em',textTransform:'uppercase'}}>Filtraciones e incidentes</div>
      ...
      {showNew&&(
        <div style={{background:C.surface2,border:`1px solid ${C.amber}44`,borderRadius:8,padding:14,marginBottom:14,animation:'slideUp 0.25s ease'}}>
          <div className="mono" style={{fontSize:11,color:C.amber,letterSpacing:'0.1em',marginBottom:12,textTransform:'uppercase'}}>Nuevo Evento</div>
          ...
          <div style={{display:'flex',gap:8}}>
            ...
            <button onClick={createEvento} disabled={!plantId||!severity||!title||saving}
              style={{flex:1,background:plantId&&severity&&title&&!saving?C.amber:C.border,color:plantId&&severity&&title&&!saving?C.bg:C.muted,border:'none',borderRadius:10,padding:'14px',fontFamily:'IBM Plex Mono',fontSize:12,fontWeight:700,minHeight:44,transition:'all 0.2s'}}>
      ...
      {filtered.length===0&&<div style={{textAlign:'center',color:C.muted,padding:'40px 0',fontSize:13}}>No hay eventos en este filtro</div>}
          ...
          <div key={t.id} style={{background:C.surface2,border:`1px solid ${urgent?C.red+'66':C.border}`,borderRadius:8,padding:14,marginBottom:10,animation:`fadeIn ${0.3+i*0.06}s ease`,position:'relative'}}>
            ...
            {t.assigned_to&&(
              <div className="mono" style={{fontSize:10,color:C.muted,marginTop:6}}>
                Asignado a: <span style={{color:C.text}}>{usuarios.find(u=>u.id===t.assigned_to)?.full_name||t.assigned_to}</span>
              </div>
            )}
      ...
      {editTicket&&<EditEventoModal ticket={editTicket} onClose={()=>setEditTicket(null)} onSaved={(updated)=>{setTickets(prev=>prev.map(t=>t.id===updated.id?{...t,...updated}:t));setEditTicket(null)}}/>}
    </div>
  )
}
...
function InspeccionesTab(){
  ...
  return(
    <div>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:14}}>
        <div className="mono" style={{fontSize:10,color:C.muted,letterSpacing:'0.1em',textTransform:'uppercase'}}>Historial de inspecciones</div>
        <Btn small onClick={()=>navigate('/gestion/inspeccion/nueva')}>+ NUEVA</Btn>
      </div>

      {inspecciones.length===0&&(
        <div style={{textAlign:'center',padding:'40px 0'}}>
          <div style={{fontSize:32,marginBottom:12}}>📋</div>
          <div style={{fontSize:14,color:C.muted,marginBottom:16}}>No hay inspecciones registradas</div>
          <Btn onClick={()=>navigate('/gestion/inspeccion/nueva')}>+ CREAR PRIMERA INSPECCIÓN</Btn>
        </div>
      )}
      ...
      {inspecciones.map((insp,i)=>{
        ...
        return(
          <div key={insp.id} onClick={()=>navigate(`/gestion/inspeccion/${insp.id}`)}
            style={{background:C.surface2,border:`1px solid ${C.border}`,borderRadius:8,padding:14,marginBottom:8,cursor:'pointer',animation:`fadeIn ${0.3+i*0.06}s ease`}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:6}}>
              <div style={{fontSize:13,fontWeight:600,flex:1,paddingRight:8}}>{insp.plants?.name||'—'}</div>
              <div style={{background:color+'22',border:`1px solid ${color}44`,borderRadius:6,padding:'4px 10px'}}>
                <span className="mono" style={{fontSize:12,fontWeight:700,color}}>RCI {rci}</span>
              </div>
            </div>
            <div style={{display:'flex',gap:10,flexWrap:'wrap'}}>
  ...
  return(
    <div style={{padding:'16px 16px 80px',animation:'fadeIn 0.3s ease'}}>
      <div style={{marginBottom:16}}>
        <div className="mono" style={{fontSize:9,color:C.muted,letterSpacing:'0.15em',textTransform:'uppercase'}}>GESTIÓN</div>
