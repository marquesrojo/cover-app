...
export function InspectionDetail(){
  ...
  if(!insp)return<div style={{padding:24,color:C.muted}}>Inspección no encontrada</div>
  ...
  return(
    <div style={{padding:'16px 16px 80px',animation:'fadeIn 0.3s ease'}}>
      <div style={{display:'flex',alignItems:'center',gap:14,marginBottom:20}}>
        ...
        <div>
          ...
          <div style={{display:'flex',gap:6}}>
            <Badge color={color}>{rciLabel(insp.rci||0)}</Badge>
            <Badge color={insp.status==='completed'?C.green:C.amber} small>{insp.status==='completed'?'COMPLETADA':'BORRADOR'}</Badge>
          </div>
        </div>
      </div>
      <div style={{background:C.surface2,border:`1px solid ${C.border}`,borderRadius:8,padding:14,marginBottom:12}}>
        <div className="mono" style={{fontSize:10,color:C.muted,letterSpacing:'0.1em',marginBottom:8,textTransform:'uppercase'}}>Datos de la inspección</div>
      ...
      {photos.length>0&&(
        <div style={{marginBottom:12}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            {photos.map(ph=>(
              <div key={ph.id}>
                <img src={ph.public_url} alt="" style={{width:'100%',aspectRatio:'4/3',objectFit:'cover',borderRadius:8,border:`1px solid ${C.border}`}}/>
                <div className="mono" style={{fontSize:8,color:C.muted,marginTop:4,textTransform:'uppercase'}}>{ph.step}</div>
              </div>
            ))}
          </div>
        </div>
      )}
      <div style={{background:color+'11',border:`1px solid ${color}33`,borderRadius:8,padding:14}}>
      ...
      <div style={{marginTop:12,textAlign:'center'}}>
        <div className="mono" style={{fontSize:9,color:C.muted}}>ID: {id}</div>
        <div className="mono" style={{fontSize:9,color:C.muted,marginTop:2}}>Compartí esta URL para que otros usuarios con login puedan ver esta inspección</div>
      </div>
    </div>
  )
}
...
export default function InspeccionScreen(){
  ...
  const save=async()=>{
    ...
    // Subir fotos
    const uploadList=[
      {photo:membPhoto,step:'membrana'},
      {photo:unionPhoto,step:'uniones'},
      {photo:drainPhoto,step:'drenaje'},
      {photo:equipPhoto,step:'equipos'},
    ].filter(x=>x.photo?.file)

    for(const {photo,step:s} of uploadList){
      try{
  ...
  if(savedId){
    ...
    return(
      <div style={{padding:'24px 16px 80px',textAlign:'center',animation:'fadeIn 0.3s ease'}}>
        ...
        <div style={{marginBottom:8}}><Badge color={color}>{rciLabel(rci)}</Badge></div>
        <div style={{fontSize:13,color:C.muted,maxWidth:280,margin:'0 auto 24px',lineHeight:1.6}}>{rciRec(rci)}</div>
        <div style={{background:C.surface2,border:`1px solid ${C.border}`,borderRadius:8,padding:14,marginBottom:16,textAlign:'left'}}>
          <div className="mono" style={{fontSize:10,color:C.amber,marginBottom:8,letterSpacing:'0.1em'}}>INSPECCIÓN GUARDADA ✓</div>
          <div style={{fontSize:12,color:C.muted,marginBottom:4}}>Link único para compartir:</div>
        ...
        <div style={{marginTop:12}}><Btn full outline onClick={()=>navigate('/dashboard')}>IR AL DASHBOARD</Btn></div>
      </div>
    )
  }
      ...
      <div style={{padding:'16px 16px 100px'}}>
        ...
        {step===2&&(
          <div style={{animation:'fadeIn 0.3s ease'}}>
            ...
            <div className="mono" style={{fontSize:10,color:jsa.every(Boolean)?C.green:C.red,marginTop:12}}>
              {jsa.every(Boolean)?'✓ TODOS LOS CONTROLES OK':`✗ ${jsa.filter(Boolean).length}/6 COMPLETADOS`}
            </div>
          </div>
        )}
        ...
        {step===7&&(
          <div style={{animation:'fadeIn 0.3s ease',textAlign:'center',paddingTop:8}}>
            ...
            <Btn full onClick={save} disabled={saving}>{saving?'GUARDANDO Y SUBIENDO FOTOS...':'✓ CERRAR Y GUARDAR INSPECCIÓN'}</Btn>
          </div>
        )}
